...
class Reporter(object):
                ...
                for i in range(0, (source_norm_length - patch_norm_length + 1)):
                    ...
                    while patch_norm_lines[patch_line] == source_norm_lines[source_line]:
                        ...
                        if patch_line == patch_norm_length:
                            self._context_dict[patch_id].append(
                                common.ContextInfo(source_id, max(0, i - common.context_line), i, source_line,
                                                   min(source_line + common.context_line, source_norm_length - 1)))
                            exact_nmatch += 1
                            break

                        while source_line < source_norm_length - patch_norm_length and source_norm_lines[
                            source_line] == '':
                            source_line += 1
    ...
    def output(self, outfile='vul.json'):
        '''
        ...
        for patch_id, context_list in self._context_dict.items():
            ...
            for context in context_list:
                ...
                if s.file_path in vul_infos:
                    vul_infos[s.file_path].append(p.file_path)
                else:
                    vul_infos[s.file_path] = [p.file_path]
        with open(outfile, "w") as out:
            json.dump(vul_infos, out, encoding="utf-8")
        return exact_nmatch
